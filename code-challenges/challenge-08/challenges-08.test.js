...
const filterStringsWithVowels = (arr) => {
  ...
  let wordsWithVowels = arr.filter(word => {
    if (word.search(regPat) !== -1) {
      return word;
    }
  });
...
const getStatName = (arr, minBaseStat) => {
  const result = getBaseStatGreaterThan(arr, minBaseStat).map(item => {
...
const getCharactersWithoutChildren = (arr) => {
  let charactersThatHaveNoKids = arr.filter((item) => !item.hasOwnProperty('children'));
  return charactersThatHaveNoKids;
};
/* ------------------------------------------------------------------------------------------------
